            ...
            <el-row>
                <el-col :span="6">
                    <el-form-item size="small" prop="list_name" label = '产品名称：'>
                        <el-input style="width:100%;" v-model="queryItem.name"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="6">
                    <el-form-item prop='descript' size = 'small' label = '产品描述：'>
...
export default {
    ...
    methods:{
        ...
        addProduct: function(item){
            console.log('item',item,item.recommend);
                ...
                this.$rq.changeProductTypeStatus(requestData).then(res=>{
                    if(res){
                        this.$message.success(item.recommend ? '取消推荐成功' : '添加推荐成功')
        ...
        getProductList: function(page){
            ...
            this.tableLoading = true
            this.$rq.getAllProductTypeList(requestData).then(res=>{
                this.tableLoading = false
                if(res){
